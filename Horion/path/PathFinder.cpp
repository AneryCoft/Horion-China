...
PathFinder::PathFinder(vec3_ti start, C_BlockSource* reg, std::shared_ptr<PathGoal> goal) : startPos(start), region(reg), goal(goal) {
}
...
struct Node {
	...
	struct {
		...
		PathSegmentType edgeType = PathSegmentType::INVALID;
	} cameFrom;
	bool isClosed;
	bool isInOpenSet;
...
struct Edge {
	...
	Edge(const NodeRef& startNode, const NodeRef& endNode, float cost, PathSegmentType type) : startNode(startNode), endNode(endNode), cost(cost), type(type) {}
};
...
__forceinline bool isDangerous(const vec3_ti& pos, C_BlockSource* reg, bool allowWater) {
	...
	// contact damage based
	{
		...
		static std::vector<const char*> dangerousTiles{ "sweet_berry_bush", "magma", "wither_rose", "cactus", "web", "lava", "ladder", "vine", "coral_block" };
...
__forceinline bool isObstructedPlayer(const vec3_ti& pos, C_BlockSource* reg, bool allowWater = false) {
	return isObstructed(pos, reg, allowWater) || isObstructed(pos.add(0, 0.5625, 0), reg) || isObstructed(pos.add(0, 1, 0), reg);
}
	...
	if (isInWater) {
		{
			...
			if (block->toLegacy()->material->isLiquid && !block->toLegacy()->material->isSuperHot) {
				if (!isObstructed(startNode.pos.add(0, 1, 0), reg, true) && !isObstructed(startNode.pos.add(0, 2, 0), reg, true))
					edges.emplace_back(startNodeRef, findNode(allNodes, mod), 0.01, PathSegmentType::WATER_WALK);
			}
		}
		if (!isObstructed(startNode.pos.add(0, -1, 0), reg, true) && canStandOn(startNode.pos.add(0, -2, 0), reg, true)) {
			auto mod = startNode.pos.add(0, -1, 0);
			edges.emplace_back(startNodeRef, findNode(allNodes, mod), 1 / WATER_SINK_SPEED, PathSegmentType::WATER_WALK);
		}
	}
	...
	for (int x = -1; x <= 1; x++) {
		for (int z = -1; z <= 1; z++) {
			...
			if (isObstructed(newPos, reg, true)) {
				...
				edges.emplace_back(startNodeRef, findNode(allNodes, newPos), JUMP_TIME, isInWater ? PathSegmentType::WATER_WALK : PathSegmentType::JUMP);
				continue;
			}
				...
				while (dropLength < 10) {
					...
					edges.emplace_back(startNodeRef, findNode(allNodes, dropPos), dropTime, PathSegmentType::DROP);
					dropLength = -1;
					break; // Also allow parkour jump
				}
				if (dropLength == 10) { // no drop found, lets try water drops
			...
			edges.emplace_back(startNodeRef, findNode(allNodes, newPos), cost, isInWater ? PathSegmentType::WATER_WALK : PathSegmentType::WALK);
		}
	}
...
Path PathFinder::findPath() {
	...
	while (!openSet.empty()) {
		...
		if (this->goal->isInGoal(cur.pos) || numNodes % 1200 == 0) {
			...
			this->currentPath = Path(segments, false);
...
const Path& PathFinder::getCurrentPath() const {
	return currentPath;
}