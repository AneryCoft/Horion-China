...
public:
	...
	virtual void flushText(float timeSinceLastFlush);
		virtual __int64 drawImage(const C_TexturePtr* texturePtr, vec2_t const& ImagePos, vec2_t const& ImageDimension, vec2_t const& idk, vec2_t const& idk2);
	virtual void drawNineslice(C_TexturePtr* texturePtr, void* nineslice);
	virtual __int64 flushImages(MC_Color& color, float alpha, class HashedString& hashedString);
